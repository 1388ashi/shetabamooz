...
@section('styles')
    ...
    <link rel="stylesheet"
        href="{{ asset('assets/PersianDateTimePicker-bs4/dist/jquery.md.bootstrap.datetimepicker.style.css') }}">
    ...
    <div class="page-header">
        ...
        <div class="mt-3 mt-lg-0">
            <div class="d-flex align-items-center flex-wrap text-nowrap">
                <a href="{{ route('admin.bootcamp-faqs.create', ['bootcamp' => request()->route('bootcamp')]) }}"
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="example-2" class="table table-striped table-bordered text-nowrap text-center">
                            <thead>
                                <tr>
                                    <th class="wd-20p border-bottom-0" style="width: 5%;"><input type="checkbox"
                                            id="check_all"></th>
                            ...
                            <tbody>
                                @forelse($faqs as $faq)
                                    <tr>
                                        ...
                                        <td>{{ $faq->id }}</td>
                                        <td>
                                            <p>{{ Str::limit($faq->question, 30) }}</p>
                                        </td>
                                        <td>
                                            <p>{{ Str::limit($faq->answer, 30) }}</p>
                                        </td>
                                        {{--                                    <td>@include('components.category', ['category' => optional($faqs->category)->name])</td> --}}
                                        <td>@include('components.bootcamp', [
                                            'bootcamp' => $faq->bootcamp->title,
                                        ])</td>

                                        <td>
                                            {{-- show --}}
                                            <a href="{{ route('admin.bootcamp-faqs.show', $faq->id) }}"
                                                data-original-title="نمایش" data-toggle="tooltip"
                                                class="btn btn-primary btn-sm"><i class="fa fa-eye"></i></a>
                                            {{-- eidt --}}
                                            <a href="{{ route('admin.bootcamp-faqs.edit', $faq->id) }}"
                                                data-original-title="ویرایش" data-toggle="tooltip"
                                                class="btn btn-warning btn-sm"><i class="fa fa-pencil"></i></a>
                                            {{-- Delete --}}
                                            <button class="btn btn-danger btn-sm text-white" data-original-title="حذف"
                                                data-toggle="tooltip"
                    ...
                    swal({
                        ...
                        .then((willDelete) => {
                            if (willDelete) {
                                ...
                                $.ajax({
                                    url: '{{ route('admin.faqs.multipleDelete') }}' +
                                        '?_token=' + '{{ csrf_token() }}',
                                    type: 'DELETE',
                                    headers: {
