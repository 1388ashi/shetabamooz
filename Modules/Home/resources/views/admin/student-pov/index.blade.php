    ...
    <div class="page-header">
        ...
        <div class="mt-3 mt-lg-0">
            <div class="d-flex align-items-center flex-wrap text-nowrap">
                <a href="{{ route('admin.student-povs.create')}}" class="btn btn-twitter">
                    ثبت نظر جدید
            ...
            <div class="card">
                ...
                <div class="card-header">
                    <div class="card-title" style="font-size: 16px;font-weight: bold;">
                        لیست همه نظرات ({{ $studentPovs->total() }})
                    </div>
                    <!--search-->
{{--                    <form method="get" id='basicSearch' action="{{route('admin.student-povs.index')}}"--}}
{{--                          autocomplete="off"--}}
                    ...
                    <div class="table-responsive">
                        <table id="example-2" class="table table-striped table-bordered text-nowrap text-center">
                            ...
                            <tr>
                                ...
                                <th class="wd-25p border-bottom-0" style="width: 10%;">وضعیت</th>
                                <th class="wd-25p border-bottom-0" style="width: 10%;">@sortablelink('comment', 'نظر')</th>
                                <th class="wd-10p border-bottom-0" style="width: 10%;">عملیات</th>
                            </tr>
                            </thead>
                            <tbody>
                            <div class="infinite-scroll">
                            @forelse($studentPovs as $i =>$studentPov)
                                <tr>
                                    <td><input type="checkbox" class="checkbox" data-id="{{$studentPov->id}}"></td>
                                    <td>{{ ++$i }}</td>
                                    <td>{{ $studentPov->name }}</td>
                                    <td><img src="{{ $studentPov->image }}" alt="{{ $studentPov->image }}" height="50px"></td>
                                    <td>@include('components.status', ['status' => $studentPov->status])</td>
                                    <td>{{ Str::limit($studentPov->comment, 30) }}</td>
                                    <td>
                                        <a href="{{route('admin.student-povs.edit',$studentPov->id)}}" data-original-title="ویرایش" data-toggle="tooltip" class="btn btn-warning btn-sm"><i class="fa fa-pencil"></i></a>
                                        {{-- Delete --}}
                                        <button class="btn btn-danger btn-sm text-white" data-original-title="حذف" data-toggle="tooltip"
                                                onclick="confirmDelete('delete-{{ $studentPov->id }}')"{{ $studentPov->isDeletable() ? '' : ' disabled' }}>
                                            <i class="fa fa-trash-o"></i></button>
                                        <form action="{{route('admin.student-povs.destroy',$studentPov->id)}}"
                                              method="post" id="delete-{{ $studentPov->id }}" style="display: none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
                    ...
                    swal({
                        ...
                        .then((willDelete) => {
                            if (willDelete) {
                                ...
                                $.ajax({
                                    url: '{{ route('admin.student-pov.multipleDelete') }}' + '?_token=' + '{{ csrf_token() }}',
                                    type: 'DELETE',
